...
Scheduled_Event = collections.namedtuple(
    "Scheduled_Event", "size time started mogi_channel"
)
...
class Mogi(commands.Cog):
    ...
    async def scheduler_mogi_start(self):
        ...
        unix_now = await self.convert_datetime_to_unix_timestamp(
        ...
        with DBA.DBAccess() as db:
            data = db.query(
                "SELECT id, mogi_format, start_time, queue_time FROM sq_schedule;", ()
            )
        for event in data:
            if event[3] < unix_now:
                logging.warning(
                    f"POP_LOG | SQ gathering! | db event time < unix_now | {event[3]} < {unix_now}"
                )
                ...
                if self.gathering:
                    ...
                    await mogi_channel.send(
                        f"Because there is an ongoing event right now, the following event has been removed: {removed_mogi_text}"
                    )
                else:
                    if self.started:
                        await self.endMogi()
                    ...
                    logging.warning(
                        f"POP_LOG | SQ scheduler_mogi_start | db event time > start_time | {event[2]} -> {start_time}"
                    )
    ...
    async def ongoing_mogi_checks(self):
        ...
        if (
            self.start_time - QUEUE_OPEN_TIME + JOINING_TIME + EXTENSION_TIME
        ) <= cur_time:
            logging.warning(f"MAKING ROOMS")
            await self.makeRoomsLogic(
                self.mogi_channel, (self.start_time.minute) % 60, True
            )
            return
        ...
        if self.start_time - QUEUE_OPEN_TIME + JOINING_TIME <= cur_time:
            ...
            if numLeftoverTeams == 0:
                ...
                if int(cur_time.second / 20) == 0:
                    ...
                    logging.warning(
                        f"NOT ENOUGH TEAMS - alert alert DJ crazy times. if you want parties to be making? have some noise..."
                    )
                    await self.mogi_channel.send(
                        f"Need {x_teams} more team(s) to start immediately. Starting in {minutes_left} minute(s) regardless."
                    )
    ...
    async def is_gathering(self, ctx):
        if self.gathering == False:
            await (
                await ctx.send(
                    "Mogi is closed; players cannot join or drop from the event"
                )
            ).delete(delay=5)
            raise Exception()
    ...
    async def can(self, ctx, members: commands.Greedy[discord.Member]):
        ...
        try:
            if len(members) > 0 and len(members) < self.size - 1:
                await self.queue_or_send(
                    ctx,
                    "%s didn't tag the correct number of people for this format (%d)"
                    % (ctx.author.display_name, self.size - 1),
                    delay=5,
                )
                return
            ...
            if checkWait is not False:
                if self.waiting[checkWait][ctx.author][0] == True:
                    await self.queue_or_send(
                        ctx,
                        "%s has already confirmed for this event; type `!d` to drop"
            ...
            if len(members) == 0:
                ...
                if checkWait is not False:
                    ...
                    string = "%s has confirmed for their squad [%d/%d]\n" % (
                        ctx.author.display_name,
                        len(confirmedPlayers),
                        self.size,
                    )
                    if len(missingPlayers) > 0:
                        string += "Missing players: "
                        string += ", ".join(
                            [player.display_name for player in missingPlayers]
                        )
                    ...
                    if len(missingPlayers) == 0:
                        ...
                        if len(self.list) > 1:
                            ...
                            s = ""
                        string += (
                            "`Squad successfully added to mogi list [%d team%s]`:\n%s"
            ...
            logging.warning(
                f"Canning up failed for {ctx.author} ({ctx.author.id}): {e}"
            )
        ...
        for member in members:
            ...
            if checkWait is not False or checkList is not False:
                ...
                if checkWait is not False:
                    ...
                    msg += ", ".join(
                        [player.display_name for player in self.list[checkList].keys()]
                    )
            ...
            if member == ctx.author:
                await self.queue_or_send(
                    ctx,
                    "%s, Duplicate players are not allowed for a squad, please try again"
                    % (ctx.author.mention),
                )
                return
        ...
        try:
            ...
            await self.queue_or_send(
                ctx,
                f"MMR not found for one of the following: {lookupMembers}",
                delay=10,
            )
            logging.warning(
                f"{ctx.author} ({ctx.author.id}) failed to can up because: {e}"
            )
            return
        ...
        for i in range(self.size - 1):
            ...
            if playerMMRs[i + 1] is False:
                await self.queue_or_send(
                    ctx,
                    "Error: MMR for player %s cannot be found! Please contact a staff member for help"
    ...
    async def drop(self, ctx):
        ...
        if checkWait is False and checkList is False:
            await self.queue_or_send(
                ctx,
                "%s is not currently in a squad for this event; type `!c @partnerNames`"
                % (ctx.author.display_name),
                delay=5,
            )
            return
    ...
    async def remove(self, ctx, num: int):
        ...
        if num > len(self.list) or num < 1:
            await (
                await ctx.send(
                    "Invalid squad ID; there are %d squads in the mogi" % len(self.list)
                )
            ).delete(delay=10)
            return
    ...
    async def launch_mogi(
        ...
        await mogi_channel.send(
            "A %dv%d mogi has been started - @here Type `!c`, `!d`, or `!list`"
            % (size, size)
        )
    ...
    async def close(self, ctx):
        ...
        self.is_automated = False
        await ctx.send(
            "Mogi is now closed; players can no longer join or drop from the event"
        )
    ...
    async def open(self, ctx):
        ...
        if self.gathering is True:
            await (
                await ctx.send(
                    "Mogi is already open; players can join and drop from the event"
                )
            ).delete(delay=5)
            return
        self.gathering = True
    ...
    async def list(self, ctx, mmrorder=""):
        ...
        if len(mogilist) == 0:
            await (
                await ctx.send(
                    "There are no squads in the mogi - confirm %d players to join"
                    % (self.size)
                )
            ).delete(delay=5)
            return
        ...
        if len(self.list) % (12 / self.size) != 0:
            msg += "`[%d/%d] teams for %d full rooms`" % (
                (len(mogilist) % (12 / self.size)),
                (12 / self.size),
                int(len(mogilist) / (12 / self.size)) + 1,
            )
        await ctx.send(msg)
    ...
    async def makeRoomsLogic(
        self,
        mogi_channel: discord.TextChannel,
        openTime: int,
        startedViaAutomation=False,
    ):
        """Sorts squads into rooms based on average MMR, creates room channels and adds players to each room channel"""
        if (
            self.making_rooms_run and startedViaAutomation
        ):  # Reduce race condition, but also allow manual !makeRooms
            return
        ...
        if numRooms == 0:
            await mogi_channel.send(
                "Not enough players to fill a room! Try this command with at least %d teams"
                % int(12 / self.size)
            )
            return
        ...
        if openTime >= 60 or openTime < 0:
            await mogi_channel.send(
                "Please specify a valid time (in minutes) for rooms to open (00-59)"
            )
            return
        penTime = openTime + 5
        ...
        for i in range(int(numRooms / catNum) + 1):
            cat = await mogi_channel.guild.create_category_channel(
                name="Rooms %d" % (i + 1), position=config["channel_category_position"]
            )
        ...
        try:
            ...
            for category in self.categories:
                try:
                    with DBA.DBAccess() as db:
                        db.execute(
                            "INSERT INTO sq_helper (category_id) values (%s)",
                            (category.id,),
                        )
                except Exception as e:
                    logging.warning(f"ERROR inserting category to sq_helpers | {e}")
        except Exception as e:
        ...
        for i in range(numRooms):
            ...
            overwrites = {
                mogi_channel.guild.default_role: discord.PermissionOverwrite(
                    read_messages=False
                ),
            ...
            for j in range(int(12 / self.size)):
                ...
                msg += "`%d.` " % (j + 1)
                msg += ", ".join(
                    [player.display_name for player in sortedTeams[index].keys()]
                )
                msg += " (%d MMR)\n" % sortedMMRs[index]
            ...
            for j in range(int(12 / self.size)):
                index = int(i * 12 / self.size + j)
                mentions += " ".join(
                    [player.mention for player in sortedTeams[index].keys()]
                )
                mentions += " "
                scoreboard += ",".join(
                    [player.display_name for player in sortedTeams[index].keys()]
            ...
            roomMsg += mentions
            try:
                roomChannel = await category.create_text_channel(
                    name=roomName, overwrites=overwrites
                )
        ...
        if numTeams < len(self.list):
            ...
            for i in range(len(missedTeams)):
                msg += "`%d.` " % (i + 1)
                msg += ", ".join(
                    [player.display_name for player in missedTeams[i].keys()]
                )
                msg += " (%d MMR)\n" % missedMMRs[i]
            await mogi_channel.send(msg)
            ...
            if current_channel == channel:
                if not isFinished:
                    await current_channel.edit(
                        name=current_channel.name + CHECKMARK_ADDITION
                    )
    ...
    async def start_input_validation(ctx, size: int):
        ...
        if size not in valid_sizes:
            await (
                await ctx.send(
                    "The size you entered is invalid; proper values are: 2, 3, 4"
                )
            ).delete(delay=5)
            return False
        return True
    ...
    @commands.guild_only()
    async def add_template_mogi(
        self, ctx, day_of_week: int, size: int, *, schedule_time: str
    ):
        await Mogi.hasroles(self, ctx)
        logging.info(
            f"add_template_mogi | day: {day_of_week}, size: {size}, date/time:{schedule_time}"
        )
        ...
        if mogi_channel is None:
            await ctx.send(
                "I can't see the mogi channel, so I can't schedule this template event."
            )
            return
        ...
        if match:
            ...
            await ctx.send(
                "I couldn't understand the time format. Please use either AM/PM or military time (e.g., '2pm' or '14')."
            )
            return
        ...
        try:
            ...
            with DBA.DBAccess() as db:
                db.execute(
                    "INSERT INTO sq_default_schedule (start_time, mogi_format, mogi_channel, day_of_week) VALUES (%s, %s, %s, %s);",
                    (schedule_time, size, mogi_channel.id, day_of_week),
                )

            await ctx.send(
                f"Templated {size}v{size} on day {day_of_week} @ {schedule_time}"
            )
            logging.info("_____________________________")
        # except (ValueError, OverflowError):
        except Exception as e:
            await ctx.send(
                "I couldn't figure out the date and time for your event. Try making it a bit more clear for me."
            )
    ...
    async def schedule(self, ctx, size: int, *, schedule_time: str):
        ...
        if mogi_channel == None:
            await ctx.send(
                "I can't see the mogi channel, so I can't schedule this event."
            )
            return
        ...
        try:
            ...
            logging.warning(
                f"POP_LOG | SQ !schedule | schedule_time type: {type(schedule_time)} | {schedule_time} UTC"
            )
            logging.warning(
                f"POP_LOG | SQ !schedule | input_time type: {type(input_time)} | {input_time}"
            )
            logging.warning(
                f"POP_LOG | SQ !schedule | queue time type: {type(queue_time)} | {queue_time}"
            )
            ...
            logging.warning(
                f"POP_LOG | SQ !schedule | input unix time: {input_unix_time}"
            )
            logging.warning(
                f"POP_LOG | SQ !schedule | queue unix time: {queue_unix_time}"
            )

            # Add to DB SQ Schedule
            with DBA.DBAccess() as db:
                db.execute(
                    "INSERT INTO sq_schedule (start_time, queue_time, mogi_format, mogi_channel) VALUES (%s, %s, %s, %s);",
            ...
            try:
                await guild.create_scheduled_event(
                    name=f"SQ:{str(size)}v{str(size)} Gathering",
            ...
            await ctx.send(
                f"Scheduled {size}v{size} on <t:{str(int(input_unix_time))}:F>"
            )
    ...
    async def parsetime(self, ctx, *, schedule_time: str):
        try:
            ...
            await ctx.send(
                f"```<t:"
                + str(int(time.mktime(actual_time.timetuple())))
                + ":F>``` -> <t:"
                + str(int(time.mktime(actual_time.timetuple())))
                + ":F>"
            )
    ...
    async def post_schedule(self, ctx):
        ...
        with DBA.DBAccess() as db:
            data = db.query(
                "SELECT id, mogi_format, start_time FROM sq_schedule ORDER BY start_time ASC;",
                (),
            )
        event_str = "@everyone\n"
        for d in data:
    ...
    async def view_template(self, ctx):
        ...
        with DBA.DBAccess() as db:
            data = db.query(
                "SELECT id, mogi_format, start_time, day_of_week FROM sq_default_schedule ORDER BY day_of_week ASC;",
                (),
            )
        event_str = ""
        for d in data:
    ...
    async def remove_event(self, ctx, event_num: int):
        ...
        try:
            with DBA.DBAccess() as db:
                data = db.query(
                    "SELECT mogi_format, start_time FROM sq_schedule WHERE id = %s;",
                    (event_num,),
                )
                size = data[0][0]
                start_time = data[0][1]
            ...
            await ctx.send(
                f"Removed event #{event_num} | {size}v{size} on <t:{str(start_time)}:F>"
            )
        except Exception as e:
    ...
    async def remove_template(self, ctx, event_num: int):
        ...
        with DBA.DBAccess() as db:
            data = db.query(
                "SELECT mogi_format, start_time, day_of_week FROM sq_default_schedule WHERE id = %s;",
                (event_num,),
            )
            size = data[0][0]
            start_time = data[0][1]
        ...
        await ctx.send(
            f"Removed event #{event_num} | {size}v{size} on day {day_of_week} @{start_time}"
        )
    ...
    async def transfer_template(self, ctx):
        ...
        with DBA.DBAccess() as db:
            template_data = db.query(
                "SELECT day_of_week, start_time, mogi_format FROM sq_default_schedule ORDER BY day_of_week ASC;",
                (),
            )
        ...
        for event in template_data:
            ...
            if days_until_target >= 0 and days_until_target <= 6:
                ...
                input_unix_time = await self.convert_datetime_to_unix_timestamp(
                    input_time
                )
                queue_unix_time = await self.convert_datetime_to_unix_timestamp(
                    queue_time
                )
                ...
                with DBA.DBAccess() as db:
                    schedule_data = db.query(
                        "SELECT start_time FROM sq_schedule WHERE start_time = %s;",
                        (input_unix_time,),
                    )
                if schedule_data:
                    await ctx.send(
                        f"Already scheduled {size}v{size} on <t:{str(int(input_unix_time))}:F>"
                    )
                    continue
    ...
    async def get_event_string(self, event_id):
        try:
            with DBA.DBAccess() as db:
                data = db.query(
                    "SELECT id, mogi_format, start_time FROM sq_schedule WHERE id = %s;",
                    (event_id,),
                )
    ...
    async def convert_datetime_to_unix_timestamp(self, datetime_object):
        return int(
            (datetime_object - datetime(1970, 1, 1, tzinfo=pytz.utc)).total_seconds()
        )
    ...
    async def send_to_debug_channel(self, ctx, error):
        ...
        embed = discord.Embed(
            title="Error", description=">.<", color=discord.Color.blurple()
        )
    ...
    async def send_raw_to_debug_channel(self, anything, error):
        channel = self.bot.get_channel(secretly.debug_channel)
        embed = discord.Embed(
            title="Raw SQ Error", description="@_@", color=discord.Color.yellow()
        )
    ...
    async def edit_sq_message(self, ctx, *, my_message: str):
        ...
        """
        try:
            sq_helper_channel = self.bot.get_channel(
                SQ_HELPER_CHANNEL_ID
            )  # Fetches the SQ helper channel
            if not sq_helper_channel:
                await ctx.send("Error: SQ helper channel not found.")
                return

            message = await sq_helper_channel.fetch_message(
                CATEGORIES_MESSAGE_ID
            )  # Fetches the message to edit
            await message.edit(
                content=str(my_message)
